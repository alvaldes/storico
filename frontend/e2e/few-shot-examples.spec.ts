...
test.describe('Few-Shot Examples in Workspace Settings', () => {
  ...
  test('Max 3 examples enforced - add button disabled', async ({ page }) => {
    ...
    for (let i = 0; i < 3; i++) {
      await page.click('text=Add Example');
      await page.fill(
        `textarea[placeholder*="As a user"] >> nth=${i}`,
        `Example ${i} user story with enough characters`,
      );
      await page.fill(
        `textarea[placeholder*="1. summary:"] >> nth=${i}`,
