...
export function localizedPath(path: string, locale: Locale): string {
  // Clamp to a known locale so this can only ever emit an internal
  // "/en..." or "/es..." path — never a protocol-relative URL.
...
export function isPublicPagePath(pathname: string): boolean {
  return [...PUBLIC_PATHS, ...PROTECTED_PATHS].some((p) => pathname === p || pathname === p + '/');
}
...
export function isProtectedPagePath(pathname: string): boolean {
  ...
  return PROTECTED_PATHS.some((p) => path === p || path === p + '/');
}
