        ...
        {value && (
          <div className="flex items-center gap-3 rounded-lg border border-border bg-muted/30 px-3 py-2 text-sm">
            ...
            <span className="text-muted-foreground">
              ...
              <span className="font-medium text-foreground">{iconNameToLabel(value)}</span>
            </span>
            <span className="ml-auto text-xs text-muted-foreground/60">{value}</span>
          </div>
        )}
      </DialogContent>
    </Dialog>
...
export function IconTrigger({ value, onClick, className, locale = 'en' }: IconTriggerProps) {
  ...
  return (
    ...
    >
      {value ? <IconDisplay name={value} className="size-4" /> : <SearchIcon className="size-4" />}
    </button>
  );
}
