...
export function ThemeToggle({ locale = 'en' }: { locale?: Locale }) {
  ...
  const Icon = mounted ? (theme === 'light' ? Moon : theme === 'dark' ? Sun : Monitor) : Sun;
    ...
    <button
      ...
      aria-label={
        mounted
          ...
          : (t.theme?.loadingToggle ?? 'Loading theme toggle')
      }
      {...(mounted
        ? {
            title: (t.theme?.currentTheme ?? 'Current: {theme}. Click to switch.').replace(
              '{theme}',
              theme,
            ),
          }
