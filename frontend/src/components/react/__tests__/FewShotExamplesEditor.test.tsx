...
describe('FewShotExamplesEditor', () => {
  ...
  it('renders existing examples', () => {
    ...
    expect(
      screen.getByDisplayValue('As a user, I want to log in so that I can access my account'),
    ).toBeInTheDocument();
  ...
  it('removes example when remove button clicked', () => {
    ...
    expect(onChange).toHaveBeenCalledWith([{ userStory: 'Example 2', tasks: 'Tasks 2' }]);
  });
  ...
  it('disables add button when max examples reached', () => {
    ...
    expect(screen.getByText('Maximum of 3 few-shot examples reached')).toBeInTheDocument();
  });
  ...
  it('shows validation error for user story too short', () => {
    ...
    expect(screen.getByText('User Story must be at least 10 characters')).toBeInTheDocument();
    expect(onChange).toHaveBeenCalledWith([
  ...
  it('shows validation error for tasks too short', () => {
    ...
    expect(screen.getByText('Tasks must be at least 20 characters')).toBeInTheDocument();
  });
  ...
  it('shows format hint in tasks textarea placeholder', () => {
    ...
    const tasksInput = screen.getAllByPlaceholderText(/1\. summary:/)[0];
    expect(tasksInput).toHaveAttribute('placeholder', expect.stringContaining('1. summary:'));
  });
