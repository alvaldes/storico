...
| F13 | **A second writer overwrites every RAG point with an empty model.** `infrastructure/tasks/extraction_task.py:542` has a module-level `_store_rag` that hardcodes `model_used=""` and writes to the same point id (`str(extraction_id)`) as `ExtractionService._store_rag` (`domain/services/extraction_service.py:359`, which passes `extraction.model_used = config.model`). Last writer wins, so **every** extraction-stored point in the live collection carries `model_used: ""` while its `extractions` row records `llama3.1:8b` or `gemini-2.5-flash` — measured on both this run's points and the previous session's. Each extraction is also embedded and upserted twice for the same point. Pre-existing and outside this feature's diff; recorded as a follow-up, not a blocker. | point payloads for workspace `01a0c6ac-…` and `01a0c6b3-…`: `model_used: ""`; `extractions.model_used = llama3.1:8b` |
