...
| D10 | Auto-extraction after import | **Out of scope for v1.** The endpoint contract is designed so adding it later does not change the response shape. It needs its own concurrency-limit decision and its own conversation, not a paragraph hidden here. **Confirmed by the maintainer** before PR 2: import creates stories in `pending_extraction` and extraction stays manual, story by story. The checkbox lands as its own task, off by default, together with the concurrency limit it requires |
| D11 | Status when nothing was created | **`201` whenever there are no blocking errors**, including a re-upload that creates nothing (`created: 0`, everything in `duplicates`). `200` would be finer-grained HTTP semantics at the cost of two success paths in the client, and the assertion the response actually makes is "the file was accepted". **Confirmed by the maintainer**; the client branches on `created`/`skipped`, never on the status |
| D12 | When the prompt-text parity is pinned | The mirror test that anchors `CANONICAL_RAW_TEXT_TEMPLATE` against `StoryForm`'s `generatedRaw` is **task 7, in PR 2** — it is a Vitest file, so it belongs with the frontend that consumes the string. **Confirmed by the maintainer.** Residual risk accepted knowingly: until PR 2 lands, editing the canonical text on either side breaks no test. Both sides were compared once by executing the TypeScript, but that is a checked coincidence, not a contract |
