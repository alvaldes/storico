...
| El deploy corre las migraciones | 🔲 | `.github/workflows/deploy-backend.yml` hace `git reset --hard` y `docker run`, y **cero `alembic`**. Ese es el mecanismo que produjo el incidente del 2026-09-20: el esquema quedó en `0021` contra un head `0024` y la extracción falló 56 veces con `column extractions.completed_at does not exist`. Se aplicaron a mano hasta `0024`, pero **la próxima revisión de esquema vuelve a caer en lo mismo**. No hay una posición fija que sirva: las tres revisiones más recientes piden órdenes opuestos (`0022` y `0024` el código primero, `0023` la migración primero), así que «después del reset del árbol de trabajo y antes de arrancar el contenedor» satisface una y viola las otras dos. Lo que existe ahora es **detección**: `GET /api/v1/health/ready` responde 503 cuando el esquema no está en el head del código, el deploy falla con eso y ya no reporta éxito sobre una release rota, así que la próxima revisión deja de caer en silencio en lo mismo. La política de orden sigue abierta. Ver `odd/tasks/schema-drift-gate.md`. |
