...
| Ítem | Estado | Detalle |
|------|--------|---------|
| Qdrant Cloud + adaptador de embeddings | 🟡 | **El código está; falta la configuración de prod.** El 2026-09-24 una extracción desde prod no dejó ningún punto: la VM no tiene Ollama, el embedding es global (no por workspace) y el fallo era un no-op mudo. Decidido por el operador: **Google `gemini-embedding-001`** en prod, **sin fallback** en runtime, y **una colección por entorno** (`storico_extractions_dev` / `storico_extractions_prod`). El `.env` de la VM necesita las 7 variables de `docs/deployment.md`; el código ya loguea el fallo en ERROR y `GET /api/v1/health/services` expone un probe real de `embeddings`. |
